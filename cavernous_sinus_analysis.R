...
data_file <- data_file %>% mutate(across(c(`Case Number`,
                                           `Tumor only in cavernous sinus (Y=1, 2 = N)`,
                                           `Tumor epicenter in cavernous sinus (Y=1, 2 = N)`,
                                           `Tumor epicenter in middle fossa (Y=1, 2 = N)`,
                                           `Tumor epicenter in posterior fossa (Y=1, 2 = N)`,
                                           `Tumor epicenter in the infratemporal fossa (Y=1, 2 = N)`),
                                  factor))
...
data_summary_CN <- data_file %>% select(all_of(select_string_CN_full)) %>% tbl_summary(statistic = list(all_continuous() ~ "{mean} ({sd})",
                                                                                                       all_categorical() ~ "{n} / {N} ({p}%)"),
                                                                                      digits = all_continuous() ~ 2,
                                                                                      missing = "no") 
                                                                                      #missing_text = "(Missing)") 
...
data_file_stats <- data_file_stats %>% rename(t_size=`Tumor size (T, cm)`,
                                              ...
                                              id = `Case Number`,
                                              cav_only = `Tumor only in cavernous sinus (Y=1, 2 = N)`,
                                              epi_cav = `Tumor epicenter in cavernous sinus (Y=1, 2 = N)`,
                                              epi_mid = `Tumor epicenter in middle fossa (Y=1, 2 = N)`,
                                              epi_post = `Tumor epicenter in posterior fossa (Y=1, 2 = N)`,
                                              epic_infra =`Tumor epicenter in the infratemporal fossa (Y=1, 2 = N)`,
